...
export function Practice() {
  ...
  return (
    <div>
      ...
      <div id="temporizador">
        <h3 id="timer">{new Date(timer * 1000).toISOString().substr(11, 8)}</h3>
      </div>
      <section id="textContainer">
        <div id="containerContent">
          <p>{currentText}</p>
        </div>
        <input id="submitAnswer" type="text" placeholder="As palavras chave do texto são..." required />
        <div className="answerButtons">
          <button className="sendAnswer" onClick={sendAnswers}>Enviar resposta</button>
          <button className="changeText" onClick={() => changeText(practiceText)}>Mudar texto</button>
        </div>
      </section>
      <footer>
        <p>Desenvolvido por <span>Daniel de Santana</span>,<span> Marcos Emanuel </span> e <span> Melkysedeke Costa</span>.</p>
      ...
      <section id="resultContainer" className="resultContainer">
        <div className="modal">
          <h1>Resultado</h1>
          <div id="scoreContainer">
            <h2 id="scoreTitle"><span id="score">{Math.round(score)}</span>/100</h2>
          </div>
          <p className="usuario">Usuario: <span id="login">admin</span></p>
          <p className="modalParagraph">Seu nível: <span id="level">{rank}</span></p>
          <p className="Text">Texto Lido: <span id="txt">{practiceText.find(text => text.paragraph === currentText)?.textTitle}</span></p>
          <p className="temp">Tempo: <span id="time">{new Date(timer * 1000).toISOString().substr(11, 8)}</span></p>
      
          <p className="paragraphmodal">Deseja salvar o resultado em um PDF?</p>
          <div className="resultButtons">
            <button id="save" onClick={saveResult}>Quero!</button>
            <button id="dontSavePdf" onClick={dontSavePdf}>Não quero</button>
          </div>
        </div>
      </section>
    </div>
